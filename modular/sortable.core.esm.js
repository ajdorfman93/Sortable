...
function normalizeEdges(edges) {
  ...
  if (Array.isArray(edges)) {
    ...
    edges.split(/[\s,]+/).forEach(function (edge) {
      if (edge && result.hasOwnProperty(edge)) {
        result[edge] = true;
      }
    });
...
function directionFromToken(token) {
  ...
  var horizontal = 0;
  var vertical = 0;
  if (token.indexOf('e') !== -1) horizontal = 1;else if (token.indexOf('w') !== -1) horizontal = -1;
  if (token.indexOf('s') !== -1) vertical = 1;else if (token.indexOf('n') !== -1) vertical = -1;
...
function ResizePlugin() {
  ...
  Resize.prototype = {
    pointerDown: function pointerDown(_ref) {
      var evt = _ref.evt,
        originalTarget = _ref.originalTarget,
        cancel = _ref.cancel;
      if (this._activeResize) return;
      var config = this._normalizeOptions();
      if (!config.enabled) return;
      var item = closest(originalTarget, this.options.draggable, this.sortable.el, false);
      if (!item) return;
      var pointer = pointerCoords(evt);
      if (!pointer) return;
      var direction = this._resolveDirection(originalTarget, item, pointer, config);
      if (!direction) return;
      cancel();
    ...
    _normalizeOptions: function _normalizeOptions() {
      ...
      var resizeOption = this.options.resize;
      if (_typeof(resizeOption) === 'object' && resizeOption !== null) {
      ...
      if (base.handles && _typeof(base.handles) !== 'object') {
        base.handles = null;
      }
      return base;
    },
    _resolveDirection: function _resolveDirection(originalTarget, item, pointer, config) {
      var fromHandles = this._directionFromHandles(originalTarget, item, config);
      if (fromHandles) return fromHandles;
      if (config.handle) {
        var handleMatch = closest(originalTarget, config.handle, item, false);
        if (!handleMatch) return null;
        var token = handleMatch.dataset && (handleMatch.dataset.resizeDirection || handleMatch.dataset.direction);
        var dir = directionFromToken(token) || this._directionFromEdges(item, pointer, config);
        if (!dir) return null;
    ...
    _directionFromHandles: function _directionFromHandles(originalTarget, item, config) {
      var handles = config.handles;
      if (!handles) return null;
      for (var token in handles) {
        if (!handles.hasOwnProperty(token)) continue;
        var selector = handles[token];
        var match = closest(originalTarget, selector, item, false);
        if (match) {
          var dir = directionFromToken(token);
          if (!dir) return null;
          dir.handle = match;
          return dir;
        }
    ...
    _directionFromEdges: function _directionFromEdges(item, pointer, config) {
      var rect = getRect(item);
      var threshold = config.edgeThreshold || 8;
      var horizontal = 0;
      var vertical = 0;
      if (config.edges.left && pointer.clientX <= rect.left + threshold) horizontal = -1;else if (config.edges.right && pointer.clientX >= rect.right - threshold) horizontal = 1;
      if (config.edges.top && pointer.clientY <= rect.top + threshold) vertical = -1;else if (config.edges.bottom && pointer.clientY >= rect.bottom - threshold) vertical = 1;
    ...
    _startResize: function _startResize(evt, item, pointer, direction, config) {
      var rect = getRect(item);
      var computed = css(item);
      var captureEl = direction.handle || item;
      if (evt.pointerId != null && captureEl.setPointerCapture) {
        try {
          captureEl.setPointerCapture(evt.pointerId);
        } catch (_) {}
      }
      var doc = captureEl.ownerDocument || document;
      this._activeResize = {
        el: item,
        ...
        config: config,
        startPointer: _objectSpread2({}, pointer),
        startRect: rect,
    ...
    _bindListeners: function _bindListeners(evt) {
    ...
    _onPointerMove: function _onPointerMove(evt) {
      var active = this._activeResize;
      if (!active) return;
    ...
    _scheduleResize: function _scheduleResize() {
      var _this = this;
      var active = this._activeResize;
      if (!active || active.raf) return;
      var scheduler = typeof window !== 'undefined' && window.requestAnimationFrame || function (cb) {
        return setTimeout(cb, 16);
      };
      active.raf = scheduler(function () {
        active.raf = null;
        _this._commitResize();
      });
    },
    _commitResize: function _commitResize() {
      var active = this._activeResize;
      if (!active || !active.pendingEvent) return;
      var evt = active.pendingEvent;
      var pointer = pointerCoords(evt);
      if (!pointer) return;
      active.pendingEvent = null;
      var deltaX = pointer.clientX - active.startPointer.clientX;
      var deltaY = pointer.clientY - active.startPointer.clientY;
      var width = active.startRect.width;
      var height = active.startRect.height;
      var marginLeft = active.startMarginLeft;
      var marginTop = active.startMarginTop;
      var config = active.config;
      if (active.direction.horizontal) {
        var candidate = active.direction.horizontal === 1 ? active.startRect.width + deltaX : active.startRect.width - deltaX;
      ...
      if (active.direction.vertical) {
        var _candidate = active.direction.vertical === 1 ? active.startRect.height + deltaY : active.startRect.height - deltaY;
        height = clamp(_candidate, config.minHeight, config.maxHeight);
    ...
    _finishResize: function _finishResize(evt) {
      var active = this._activeResize;
      if (!active) return;
      var cancelFrame = typeof window !== 'undefined' && window.cancelAnimationFrame || function (id) {
        clearTimeout(id);
      };
      if (active.raf) {
    ...
    _emitResize: function _emitResize(name, originalEvent, width, height, deltaX, deltaY) {
      var active = this._activeResize;
      var targetEl = active ? active.el : originalEvent && originalEvent.target;
      dispatchEvent({
        sortable: this.sortable,
        rootEl: this.sortable.el,
        name: name,
...
export { AutoScrollPlugin as AutoScroll, MultiDragPlugin as MultiDrag, OnSpill, ResizePlugin as Resize, Sortable, SwapPlugin as Swap };
